...
class Pkg_PwtSitemapInstallerScript
{
	 ...
	 * @since   1.0.0
	 */
	public function addMenuTypesToPwtSiteMap($adapter)
	{
		...
		// Params
		$params                          = [];
		$params['addcontenttohtmlsitemap'] = 1;
		$params['addcontenttoxmlsitemap']  = 1;
		...
		try
		{
			...
			$query = $db->getQuery(true)
				...
				->columns(array($db->quoteName('menu_types_id'), $db->quoteName('ordering'), $db->quoteName('params')));

			foreach ($results as $key => $result)
			{
				$query->values($result . ',' . ($key + 1) . ', ' . $db->q($jsonParams));
			}
