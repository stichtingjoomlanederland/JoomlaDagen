...
class PwtSitemapModelMenu extends FormModel
{
	...
	public function save($data)
	{
		...
		$insertData = (object) [
			...
			'ordering'      => isset($data['ordering']) ? (int) $data['ordering'] : 1
		];
		...
		$app->triggerEvent('onContentBeforeSave', [$this->_context, &$table, $isNew, $data]);
		...
		$app->triggerEvent('onContentAfterSave', [$this->_context, &$table, $isNew, $data]);

		// Store the saved id to state
		$this->setState('menu.id', $table->menu_types_id);
