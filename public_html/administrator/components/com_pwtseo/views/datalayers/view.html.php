...
class PWTSEOViewDatalayers extends HtmlView
{
	...
	private function toolbar()
	{
		...
		else
		{
			...
			if ($canDo->get('core.copy'))
			{
				$childBar->save2copy('datalayer.save2copy')->listCheck(true);
			}

			if ($canDo->get('core.edit'))
			{
				$childBar->publish('datalayers.publish')->listCheck(true);
				$childBar->unpublish('datalayers.unpublish')->listCheck(true);
			}

			if ($canDo->get('core.delete'))
			{
				$childBar->trash('datalayers.delete')->listCheck(true);
			}
		}
