...
class PWTSEOModelCustom extends AdminModel
{
	 ...
	 * @since   1.1.0
	 */
	public function getTable($type = 'Custom', $prefix = 'PWTSEOTable', $config = [])
	{
	...
	public function save($data)
	{
		...
		$data['context_id'] = isset($data['id']) && $data['id'] ? $data['id'] : 0;

		$result = parent::save($data);

		if ($result)
		{
			$aDataLayers = Factory::getApplication()->input->post->get('pwtseo', [], 'array');
			...
			if (isset($aDataLayers['datalayers']))
			{
				...
				foreach ($aDataLayers['datalayers'] as $id => $values)
				{
					...
					{
						$db->updateObject('#__plg_pwtseo_datalayers_map', $item, ['context_id', 'context', 'datalayer_id']);
					}
				}
			}
		}
	...
	protected function loadFormData()
	{
		...
		//J4 if data is array convert to object
		if(is_array($data))
		{
