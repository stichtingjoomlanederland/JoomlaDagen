...
class PwtAclModelWizard extends ListModel
{
	...
	protected function createGroup($title)
	{
		...
		if (Version::MAJOR_VERSION === 4)
		{
			return (int) $groupModel->state->get('group.id');
		}
	...
	public function getItems()
	{
		...
		// We need components, otherwise start over
		if (empty($components))
		{
			Factory::getApplication()->redirect('index.php?option=com_pwtacl&view=wizard');
		}
		...
		foreach ($assets as $asset)
		{
			...
			isset($components->{$asset->component}) ?: $components->{$asset->component} = new stdClass;
			$components->{$asset->component}->assets[] = $asset;
			$components->{$asset->component}->title    = !empty($assets[$asset->component]->title) ? $assets[$asset->component]->title : '';
		}
