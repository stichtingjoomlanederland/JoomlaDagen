...
class PwtAclModelAssets extends ListModel
{
	...
	protected function getListQuery()
	{
		...
		if (Version::MAJOR_VERSION === 4)
		{
			$query
				->select(
					[
						'workflows.extension AS workflow',
						'IF(SUBSTRING_INDEX(SUBSTRING_INDEX(a.name, ".", 2), ".", -1) = "stage", workflowstages.workflow_id, 
						IF(SUBSTRING_INDEX(SUBSTRING_INDEX(a.name, ".", 2), ".", -1) = "transition", workflowtransitions.workflow_id, ""
						)) AS workflowid'
					])
				->leftJoin($db->quoteName('#__workflows', 'workflows') . ' ON workflows.asset_id = a.id')
				->leftJoin($db->quoteName('#__workflow_stages', 'workflowstages') . ' ON workflowstages.asset_id = a.id')
				->leftJoin($db->quoteName('#__workflow_transitions', 'workflowtransitions') . ' ON workflowtransitions.asset_id = a.id');
		}
		...
		$query
			...
			->where($db->quoteName('a.name') . ' NOT LIKE ' . $db->quote('com_workflow'));
	...
	public function prepareAssets($assets, $type, $group, $user)
	{
		...
		foreach ($assets as $key => $asset)
		{
			...
			if ($asset->name !== 'root.1')
			{
				...
				unset($asset->actions->core->{'core.login.api'});
			...
			switch ($asset->type)
			{
				...
				// Workflows
				case 'workflow':
					$asset->link = 'index.php?option=com_workflow&task=workflow.edit&id=' . $asset->objectid . '&extension=' . $asset->workflow;
					$asset->icon = 'file-alt';
					break;

				// States
				case 'stage':
					$asset->link = 'index.php?option=com_workflow&task=stage.edit&id=' . $asset->objectid . '&workflow_id=' . $asset->workflowid . '&extension=' . $asset->component;
					$asset->icon = 'address';
					break;

				// Transitions
				case 'transition':
					$asset->link = 'index.php?option=com_workflow&task=transition.edit&id=' . $asset->objectid . '&workflow_id=' . $asset->workflowid . '&extension=' . $asset->component;
					$asset->icon = 'address';
					break;
			}
	...
	public function getActions($assetName, $actionsOnly = false)
	{
		...
		if ($assetName === 'root.1')
		{
			$filePath = Version::MAJOR_VERSION === 4 ? 'forms/application.xml' : 'model/form/application.xml';

			$actions->items = Access::getActionsFromFile(
				JPATH_ADMINISTRATOR . '/components/com_config/' . $filePath,
