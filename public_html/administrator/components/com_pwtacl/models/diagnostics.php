...
class PwtAclModelDiagnostics extends ListModel
{
	...
	public function getDiagnosticsSteps()
	{
		...
		{
			$steps[12] = 'GENERAL_WORKFLOWS';
			$steps[13] = 'GENERAL_WORKFLOWTRANSITIONS';
			$steps[14] = 'GENERAL_WORKFLOWSTAGES';
		}

		$steps[15] = 'GENERAL_LANGUAGES';
		$steps[16] = 'ADMINCONFLICTS';
		$steps[17] = 'REBUILD';
		...
		if (Version::MAJOR_VERSION === 4)
		{
			...
			if (!$issues)
			{
				$issues = $this->checkAssets(
					...
					'com_workflow',
					'workflow',
					'SUBSTRING_INDEX(workflows.extension, ".", 1)'
				);
			}

			// Check [component].transition.[id]
			if (!$issues)
			{
				$issues = $this->checkAssets(
					false,
					...
					'com_workflow',
					'transition',
					'CONCAT(SUBSTRING_INDEX(workflows.extension, ".", 1), ".workflow.",workflows.id)'
				);
			}

			// Check [component].stage.[id]
			if (!$issues)
			{
				$issues = $this->checkAssets(
					false,
	...
	protected function getMissingAssets($table = '', $extension = '', $view = '', $parentjoin = '')
	{
		...
		switch ($table)
		{
			...
			case 'workflows':
				$name  = 'CONCAT(SUBSTRING_INDEX(' . $table . '.extension, ".", 1), ".' . $view . '.",' . $table . '.id)';
				$level = '(parentassets.level + "1")';
				break;

			case 'workflow_transitions':
			case 'workflow_stages':
				$name  = 'CONCAT(SUBSTRING_INDEX(workflows.extension, ".", 1), ".' . $view . '.",' . $table . '.id)';
				$level = '(parentassets.level + "1")';
				break;
		...
		switch ($table)
		{
			...
			case 'workflow_stages':
				$query
					->leftJoin($db->quoteName('#__workflows', 'workflows') . ' ON workflows.id = ' . $table . '.workflow_id');
				break;
		}
	...
	protected function getIncorrectAssetIds($table = '', $extension = '', $view = '')
	{
		...
		switch ($table)
		{
			...
			case 'workflows':
				$query
					->leftJoin($db->quoteName('#__assets', 'assets') . ' 
						ON assets.name = CONCAT(SUBSTRING_INDEX(' . $table . '.extension, ".", 1), ".' . $view . '.",' . $table . '.id)'
					);
				break;
			...
			case 'workflow_stages':
				$query
					->leftJoin($db->quoteName('#__workflows', 'workflows') . ' ON workflows.id = ' . $table . '.workflow_id')
					->leftJoin($db->quoteName('#__assets', 'assets') . ' 
						ON assets.name = CONCAT(SUBSTRING_INDEX(workflows.extension, ".", 1), ".' . $view . '.",' . $table . '.id)'
					);
				break;
	...
	protected function getComponents($returnquery, $table, $view = '')
	{
		...
		// Override table for workflows
		if ($table === 'workflow_transitions' || $table === 'workflow_stages')
		{
			$table = 'workflows';
		}
		...
		{
			$query
				->select('DISTINCT SUBSTRING_INDEX(extension, ".", 1)');
		}
		else
		{
			$query
