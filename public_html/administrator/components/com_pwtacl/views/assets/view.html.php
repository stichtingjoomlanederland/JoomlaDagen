...
class PwtAclViewAssets extends HtmlView
{
	...
	protected function addToolbar()
	{
		...
		if ($this->group && $canDo->get('core.edit'))
		{
			...
			{
				ToolbarHelper::custom('assets.clear', 'delete', 'delete', 'COM_PWTACL_TOOLBAR_CLEAR', false);
			}

			// Reset button for default user groups
			if ($this->group <= 9)
			{
				ToolbarHelper::custom('assets.reset', 'refresh', 'refresh', 'COM_PWTACL_TOOLBAR_REVERT', false);
			}

			// Copy button
			ToolbarHelper::modal('copyModal', 'icon-copy', 'COM_PWTACL_TOOLBAR_COPY');

			// Export button
			ToolbarHelper::custom('assets.export', 'download', 'download', 'COM_PWTACL_TOOLBAR_EXPORT', false);

			// Import button
			ToolbarHelper::modal('importModal', 'icon-upload', 'COM_PWTACL_TOOLBAR_IMPORT');
		}
