...
class PlgSystemPWTSEO extends CMSPlugin
{
	...
	public function onContentPrepareForm($form, $data)
	{
		...
		if (isset($this->aAllowedContext[$formName]) && Factory::getUser()->authorise('core.create', 'com_pwtseo'))
		{
			...
			if (Factory::getApplication()->input->get('option') === 'com_flexicontent')
			{
				$formName = 'com_flexicontent';
			}
			...
			Factory::getDocument()->addScriptOptions('PWTSeoConfig',
				[
					...
					'requirements_internal_link_bad'                 => Text::_('PLG_SYSTEM_PWTSEO_REQUIREMENTS_INTERNAL_LINKS_BAD'),
					'requirements_internal_link_good'                => Text::_('PLG_SYSTEM_PWTSEO_REQUIREMENTS_INTERNAL_LINKS_GOOD'),
					...
					'articlebodyselector'                            => $this->params->get('articlebodyselector', ''),
					'default_fallback'                               => (boolean) $this->params->get('default_fallback', ''),
					'density_min'                                    => (float) $this->params->get('density_min', '1'),
					'density_max'                                    => (float) $this->params->get('density_max', '3'),
					'pwtseoselector'                                 => $this->params->get('pwtseo_divselector', '#attrib-seofieldset'),
					'pwtseoscoreselector'                            => $this->params->get('pwtseo_divscoreselector', 'a[href="#attrib-seofieldset"]'),
					'joomla_version'                                 => Version::MAJOR_VERSION
				]
			);
		}
		...
		if ($form->getField('plg_system_pwtseo', 'params'))
		{
			...
			$styles = $this->params->get('pwtseo_divstyle', '');
			...
			if (is_object($data))
			{
				...
				if ($iId > 0)
				{
					...
					if ($this->componentParams->get('autofillmeta', 0))
					{
						...
						{
							$content = (isset($data->articletext) && $data->articletext !== '' ? $data->articletext : (isset($data->introtext) && $data->introtext ? $data->introtext : (isset($data->description) && $data->description ? $data->description : '')));
	...
	private function getSEOData($sValue, $sKey = 'context_id', $sContext = 'com_content.article')
	{
		...
		try
		{
			if (!in_array($sContext, ['com_pwtseo.custom', 'easyblog.blog']) && isset($this->aAllowedContext[$sContext]) && is_numeric($sValue))
			{
				$q
					->select($this->db->quoteName('item.language'))
					->leftJoin($this->db->quoteName($this->aAllowedContext[$sContext], 'item') . ' ON ' .
						$this->db->quoteName('item.id') . ' = ' . (int) $sValue);
			}
	...
	private function getCurrentSEOData($id = false, $context = false)
	{
		...
		if ($sContext === 'com_easyblog.entry')
		{
			$sContext = 'easyblog.blog';
		}
		...
		if (!isset($this->aSEOData[$cacheIndex]))
		{
			...
			if ($sContext === 'com_content.article'
				&& $menuItem
				&& (isset($menuItem->query['id']) && $menuItem->query['id'] ? (int) $menuItem->query['id'] : 0)   === $sId
			...
			{
				$this->aSEOData[$cacheIndex] = array_merge(
					array_filter($aCustom, static function ($e) { return isset($e) && $e !== ''; }),
					array_filter($aContent, static function ($e) { return isset($e) && $e !== ''; }),
					array_filter($aMenu, static function ($e) { return isset($e) && $e !== ''; })
				);
			}
			else
			{
	...
	public function onContentPrepare($context, &$article, &$params = false, $page = false)
	{
		...
		)
		{
			...
			{
				// Conflict with yooTheme template
				if (in_array($sKey, ['tags', 'params']))
				{
					continue;
				}
	...
	public function onContentAfterSave($context, $article, $isNew)
	{
		...
		if (isset($this->aAllowedContext[$context]) && strpos($context, 'com_pwtseo') === false)
		{
			...
			{
				// Support for Easyblog
				$aSEO = $this->app->input->post->get('pwtseo', [], 'array');
			...
			$oInput->version = preg_replace('/-\w+/', '', '2.0.1');
			$oInput->context = $context;

			$iId = $this->getHasSEOData($article->id, 'context_id', $context);
			
			// J4 Make sure we convert flag values to intergers
			$oInput->flag_outdated =  (int) $oInput->flag_outdated;
			$oInput->google_rank =  (int) $oInput->google_rank;
	...
	public function onBeforeRender()
	{
		...
		if ($doc instanceof HtmlDocument && $this->app->isClient('site') && !$this->app->input->get('customizer'))
		{
			...
			if ($aSEO || $bPreview)
			{
				if ($bPreview)
				{
					if ($input->get('context', '') === 'com_easyblog')
					{
						$post = [];
						$aSEO = $input->get('pwtseo','', 'array');
					}
					else
					{
				...
				if (isset($aSEO['page_title'], $aSEO['override_page_title']) && $aSEO['page_title'] !== '' && (int) $aSEO['override_page_title'] === 1)
				{
				...
				// With advanced mode disabled, there is no way to set canonical
				if ($overrideCanonical === 4 && !$this->params->get('advanced_mode', 1) &&  $this->params->get('set_canonical', 0)) {
					$overrideCanonical = 1;
				}
	...
	private function stripCanonicalParams($canonical, $seoData = [])
	{
		...
		else if (($params || $data) && !in_array($choice, ['1', '0', '2'], true))
		{
			$arr = explode(',', $params);
	...
	public function onAfterRender()
	{
		if ($this->app->isClient('site') && $this->app->input->getInt('pwtseo_preview', 0))
		{
			...
			if ($this->app->input->get('context', '') === 'com_easyblog')
			{
				$buffer = str_replace([
					'You are currently previewing a post that was modified. This copy is not used on the site yet unless you publish this revision',
					'Previewing Modified Post',
					'Use This Revision'
				], '', $buffer);
			}
			...
			if ($this->app->input->post->get('pwtseo_strip'))
            {
                ...
                // Avoid conflicts with BA Gallery
                $strBody = preg_replace('/\[gallery ID=+(.*?)\]/i', '', $strBody);
                $strBody = str_replace(["\n", "\t", "\r\n"], ' ', strtolower(strip_tags($strBody)));

                $buffer = str_replace(
                    '</body>',
                    '<script id="pwtseo_strbody">' . html_entity_decode($strBody). '</script></body>',
                    $buffer
                );
            }
	...
	public function preprocessBuildRule(&$router, &$uri)
	{
		...
		if ($lang === '*')
		{
			if (ComponentHelper::isInstalled('com_languages'))
			{
	...
	public function onAjaxPWTSEOPage()
	{
		...
		switch ($this->app->input->get('context'))
		{
			...
			case 'com_flexicontent':
				$aResponse = $this->processFlexicontent($aData, $iStep);
				break;
			case 'com_easyblog':
				$aData = [
					'id' => $this->app->input->get('id'),
					'pwtseo' => $this->app->input->get('pwtseo')
				];

				$aResponse = $this->processEasyBlog($aData, $iStep);
				break;
		}
	...
	protected function processArticle($aData, $iStep = 1)
	{
		...
		if ($iStep === 1)
		{
			if (Version::MAJOR_VERSION < 4)
			{
				if (isset($aData['attribs']['j2store']) && $aData['attribs']['j2store']['enabled'] === '1')
				{
					$aArr['url'] = $aArr['previewurl'] = Uri::root(
						true,
						str_replace(
							['%2F', '%3F', '%3D', '%26amp%3B'],
							['/', '?', '=', '&'],
							rawurlencode(
								urldecode(
									Route::link(
										'site',
										'index.php?option=com_j2store&view=products&id=' .
										$aData['attribs']['j2store']['j2store_product_id'] .
										'&lang=' . $aData['language']
									)
								)
							)
					...
					else
					{
						$aArr['new_url'] = Uri::root(
							...
							str_replace(
								...
								rawurlencode(
									urldecode(
										Route::_(RouteHelper::getArticleRoute($iId, (int) $aData['catid'], $aData['language']))
									)
								)
							)
						);
	...
	/**
	 * Process an Flexicontent item. Depending on the step, different data is returned because we process data in two seperate steps.
	 *
	 * @param   array  $aData  The request data
	 * @param   int    $iStep  The step to get the data for
	 *
	 ...
	 * @since   1.6.0
	 */
	protected function processFlexicontent($aData, $iStep = 1)
	{
		$aArr = [];
		$iId  = isset($aData['id']) && $aData['id'] ? (int) $aData['id'] : 0;
	 ...
	 * @param   string     $table Optional table which to check for the title
	 *
	 * @return  bool True if title is found only once, false otherwise
	 *
	 * @since   1.0.1
	 */
	protected function isTitleUnique($sTitle, $iId = 0, $table = '#__content')
	{
		$q = $this->db->getQuery(true);

		$q
			->select('COUNT(*)')
			->from($this->db->quoteName($table, 'content'))
	...
	protected function isMetaDescriptionUnique($sDescription)
	{
		...
		// Handle Easyblog
		if (ComponentHelper::isEnabled('com_easyblog'))
		{
			$q
				->clear()
				->select('COUNT(*)')
				->from($this->db->quoteName('#__pwtseo', 'content'))
				->where('LOWER(' . $this->db->quoteName('content.metadesc') . ') = ' . $this->db->quote(strtolower($sDescription)));
		}
	...
	/**
	 * Process an Easyblog Post. Depending on the step, different data is returned because we process data in two seperate steps.
	 *
	 * @param   array  $aData  The request data
	 * @param   int    $iStep  The step to get the data for
	 *
	 ...
	 * @since   1.6.0
	 */
	protected function processEasyblog($aData, $iStep = 1)
	{
		$aArr = [];
		...
		if ($iStep === 1)
		{
			...
			$query = $db->getQuery(true)
				->select(' MAX(`id`)')
				->from($db->quoteName('#__easyblog_revisions'))
				->where($db->quoteName('post_id') . ' = ' . (int) $aData['id']);

			$rId = $db->setQuery($query)->loadResult();

			$aArr['previewurl'] = str_replace(
				['%2F', '%3F', '%3D', '%26amp%3B'],
				['/', '?', '=', '&'],
				rawurlencode(
					urldecode(
						Route::link('site', 'index.php?option=com_easyblog&view=entry&layout=preview&uid=' . $aData['id'] . '.' . $rId)
					)
				)
			);
			...
			$aArr['url'] = Uri::root(
				...
				str_replace(
					...
					rawurlencode(
						urldecode(
							Route::_('index.php?option=com_easyblog&view=entry&id=' . $aData['id'])
						)
					)
				)
			);
			...
			$aArr['reachable'] = $this->isReachable($aArr['url']);
			$aArr['count']     = $this->findUsages($aData['pwtseo']['focus_word'], $aData['id']);
		}

		if ($iStep === 2)
		{
			$aArr['page_title_unique']    = $this->isTitleUnique($aData['title'], $aData['id'], '#__easyblog_post');
			$aArr['page_metadesc_unique'] = !isset($aData['metadesc']) || $this->isMetaDescriptionUnique($aData['metadesc']);
			$aArr['new_url']              = Uri::root(true, $aData['url']);

			$count         = $this->findUsages($aData['pwtseo']['focus_word'], $aData['id']);
			$aArr['count'] = $count > 1 ? $count - 1 : 0;
		}
